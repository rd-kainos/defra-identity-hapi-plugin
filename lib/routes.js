  ...
  server.route({
    ...
    handler: async (request, h) => {
      try {
        ...
        if (authorizationErr) {
          return internals.routes.handleAuthorisationError(request, h, authorizationErr)
        }

        return await internals.routes.handleValidatedToken(request, h, stateUid, savedState, tokenSet)
      } catch (e) {
        debug(e)
