const debug = require('debug')('defra.identity:internals:dynamicsHelper')
...
module.exports = (
  ...
  /**
   *
   * @param {Object<{roles: Array.<string>, mappings: Array.<string>}>} authzApiResponse
   * @return {null|Object.<string, {organisation: <Object<{id: string, name: string, roles: <Object.<string, {id: string, name: string, status: <Object.<string, {id: string, name: string}>>}>>>}>}
   */
  const parseAuthzRoles = (authzApiResponse) => {
    ...
    roles.forEach(role => {
      ...
      byStatus[orgRoleStatusId] = byStatus[orgRoleStatusId] || {}

      // Make sure we have an object for this organisation in our status roles object
      byStatus[orgRoleStatusId][orgId] = byStatus[orgRoleStatusId][orgId] || _blankParsedAuthzRecord(orgId, orgName)
      ...
      byStatus[orgRoleStatusId][orgId].roles[roleId] = orgEnrolmentRecord
