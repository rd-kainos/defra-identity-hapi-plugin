...
module.exports = (
   ...
   * @param {string} obj.journeyName - The name of the policy the user should be sent to in the identity app
   ...
   */
  const generateAuthenticationUrl = (backToPath, {policyName, journeyName, forceLogin = false, returnUrlObject = false} = {}) => {
   ...
   * @param {string} obj.policyName - The name of the policy the user should be sent to in B2C
   * @param {string} obj.journeyName - The name of the journey the user should be sent to in the identity app
   * @param {Boolean} obj.forceLogin - Whether the user should be forced to log in or not - ignores whether they are already logged in at the IdP
   */
  const generateFirstStageOutboundRedirectUrl = async ({backToPath, policyName, journeyName, forceLogin}) => {
    policyName = policyName || config.defaultPolicy
    journeyName = journeyName || config.defaultJourney

    const identityAppUrl = url.parse(config.identityAppUrl)

    identityAppUrl.pathname = '/journey'

    identityAppUrl.query = {
      returnUrl: config.returnUriFqdn,
      backToPath,
      forceLogin: forceLogin ? 'yes' : undefined,
      policyName,
      journey: journeyName
