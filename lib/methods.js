...
module.exports = (
   ...
   * @param {Boolean} obj.forceLogin - Whether the user should be forced to log in or not - ignores whether they are already logged in at the IdP
   */
  const generateFirstStageOutboundRedirectUrl = async ({ backToPath, policyName, forceLogin, journey }, { stateUid = null, stateCacheData = {} } = {}) => {
   ...
   */
  const generateFinalOutboundRedirectUrl = async ({ backToPath, policyName, forceLogin, journey }, { stateUid = null, stateCacheData = {}, returnUrl = null, clientId = null } = {}) => {
    policyName = policyName || config.defaultPolicy
    journey = journey || config.defaultJourney
    stateUid = stateUid || uuidv4()
    returnUrl = returnUrl || config.returnUriFqdn
