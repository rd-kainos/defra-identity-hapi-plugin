...
const debug = require('debug')('idm:methods')
...
module.exports = ({
  ...
  server.method('idm.getCredentials', async (request) => {
    ...
    const cacheKey = _.get(request, ['state', config.cookieName, 'oid'])
  ...
  server.method('idm.generateAuthenticationUrl', (backToPath, {policy, forceLogin = false} = {}) => {
    ...
    // @todo for association, store options (redirectTo) in cache and pass hash of record into state
