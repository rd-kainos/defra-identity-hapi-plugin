...
module.exports = (
  ...
  const readContacts = async (token, query) => {
  ...
  const readCompanyNumbers = async (token, query) => {
    const headers = buildHeaders(token)
    let url
   ...
   * @param {object} query
   */
  const readOrganisations = async (token, query) => {
    const headers = buildHeaders(token)
    let params = {}
    let url
    if (query.email) {
    ...
    const databuckets = data.map(async d => {
      ...
      const companyNumbers = await readCompanyNumbers(token, { organisationId })
  ...
  const readContactsEmployerLinks = async (token, contactId) => {
    const params = {
  ...
  const readConnectionDetails = async (token, connectionId) => {
    const headers = buildHeaders(token)
  ...
  const readServiceRoles = async (token, serviceId) => {
    const params = {}
   ...
   */
  const readEnrolment = async (token, contactId, serviceRoleIds = null, accountIds = null) => {
    const params = {}
   ...
   */
  const readServiceEnrolment = async (token, serviceId, b2cObjectId) => {
    const response = {
      roles: [],
      mappings: []
    }
    ...
    const [
      ...
      readServiceRoles(token, serviceId),
      readContacts(token, { b2cObjectId })
    ])
    ...
    const enrolment = await readEnrolment(token, contactId, serviceRoles.map(role => role.defra_lobserivceroleid))
