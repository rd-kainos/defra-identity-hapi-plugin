...
module.exports = (
  ...
  const createEnrolment = async (lobServiceRoleId, contactId, organisationAccountId, connectionDetailsId, enrolmentStatus, enrolmentType) => {
    const request = await createEnrolment.buildRequest(lobServiceRoleId, contactId, organisationAccountId, connectionDetailsId, enrolmentStatus, enrolmentType)

    return requestPromise(request).then(decodeResponse)
  }

  createEnrolment.buildRequest = async (lobServiceRoleId, contactId, organisationAccountId, connectionDetailsId, enrolmentStatus, enrolmentType) => {
    const payload = {
      ServiceRoleRef: {
