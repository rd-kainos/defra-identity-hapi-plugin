...
lab.experiment('OIDC authorisation code flow', () => {
  ...
  lab.test('The plugin should execute our final redirect function', async () => {
    ...
    const handlerResponse = await idmInternals.routes.handleValidatedToken(dummyRequest, null, '1234', {}, tokenSet)

    expect(handlerResponse).to.equal(responseIdentifier)
  })

  lab.test('The plugin should redirect our request to reset the password to the reset password policy', async () => {
    const idmConfig = server.methods.idm.getConfig()
    const idmInternals = server.methods.idm.getInternals()
